...
public class OverlayEvents {
	...
	public static void handleOverlayEvent(RenderGameOverlayEvent event) {
		...
		Object copper_watch = null;
		Object silver_watch = null;
		Object gold_watch = null;
		Object depth_meter = null;
		
		boolean depthmeter = depth_meter != null;
		...
		if (event.getType() == ElementType.HOTBAR) {
			if (Util.terrariaInventory == true) {
		        ...
		        instance.getTextureManager().bindTexture(GuiContainerTerraria.INVENTORY_BACKGROUND);
			    ...
			    for (int i = 0; i < 10; i++) {
			    	if (inventory.hotbarSelected == i) {
			    		instance.getTextureManager().bindTexture(GuiContainerTerraria.INVENTORY_BACKGROUND);
			    		GuiContainerTerraria.drawTexturedRectangle(xx + i * 22, yy, 46, 168, 22, 22);
			    	} else {
			    		instance.getTextureManager().bindTexture(GuiContainerTerraria.INVENTORY_BACKGROUND);
			    		GuiContainerTerraria.drawTexturedRectangle(xx + i * 22, yy, 23, 168, 22, 22);
			    	}
			    	if (inventory.hotbar[i].stack != null)
			    		GuiContainerTerraria.renderItemIntoGUI(inventory.hotbar[i].stack, xx + i * 22 + 2, yy + 3);
...
//		drawTexturedModalRect(xPos, yPos, 0, 0, 32, 32);
		...
		if (event.getType() == ElementType.EXPERIENCE) {
		    ...
		    if (instance.getRenderViewEntity() instanceof EntityPlayer) {
				...
				instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(xx), Conversions.toScreenY(yy), 9 * 1, 9 * 2, 9, 9);
				
				instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(xx + 10 * 5), Conversions.toScreenY(yy), 9 * 2, 9 * 2, 9, 9);
				
				instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(xx + 10 * 10), Conversions.toScreenY(yy), 9 * 3, 9 * 2, 9, 9);

				instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(xx + 10 * 15), Conversions.toScreenY(yy), 9 * 4, 9 * 2, 9, 9);

				
				EntityPlayer player = (EntityPlayer)instance.getRenderViewEntity();
				...
				if (Util.renderPotionSickness > 0) {
					instance.getTextureManager().bindTexture(new ResourceLocation("trewrite", "textures/gui/icons.png"));
					instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(5), Conversions.toScreenY(12 * 2 + 12 * debuff) + accessory * textSize, 12 * 0, 27 + 12 * 0, 12, 12); //debuff background
					instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(5), Conversions.toScreenY(12 * 2 + 12 * debuff) + accessory * textSize, 12 * 0, 27 + 12 * 1, 12, 12); //debuff image
				...
				if (Util.renderManaSickness > 0) {
					instance.getTextureManager().bindTexture(new ResourceLocation("trewrite", "textures/gui/icons.png"));
					instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(5), Conversions.toScreenY(12 * 2 + 12 * debuff) + accessory * textSize, 12 * 1, 27 + 12 * 0, 12, 12); //debuff background
					instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(5), Conversions.toScreenY(12 * 2 + 12 * debuff) + accessory * textSize, 12 * 1, 27 + 12 * 1, 12, 12); //debuff image
		...
		if (event.getType() == ElementType.FOOD) {
		    ...
		    if (instance.getRenderViewEntity() instanceof EntityPlayer) {
				...
				for (int i = 0; i < len2; i++) {
					if (i <= 10) {
						instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(X + i * 10), Conversions.toScreenY(Y), 9 * 0, 9 * 1, 9, 9);
					} else {
						instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(X + (i - 10) * 10), Conversions.toScreenY(Y - 10), 9 * 0, 9 * 1, 9, 9);
					}
				}
				...
				for (int i = 0; i < len; i++) {
					if (i <= 10) {
						instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(X + i * 10), Conversions.toScreenY(Y + addShake(shake)), 9 * 4, 9 * 1, 9, 9);
					} else {
						instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(X + (i - 10) * 10), Conversions.toScreenY(Y - 10 + addShake(shake)), 9 * 4, 9 * 1, 9, 9);
					}
				}
				if (half == true) {
		...
		if (event.getType() == ElementType.ARMOR) {
			...
			//draw armor sprite
			instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(xx), Conversions.toScreenY(yy), 9 * 0, 9 * 2, 9, 9);
		...
		if (event.getType() == ElementType.HEALTH) {
			...
			if (instance.getRenderViewEntity() instanceof EntityPlayer) {
				...
				for (int i = 0; i < len2; i++) {
					...
					if (i < 10) {
						instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(X + i * 10), Conversions.toScreenY(Y), 9 * 0, 9 * 0, 9, 9);
					} else {
						instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(X + (i - 10) * 10), Conversions.toScreenY(Y - 10), 9 * 0, 9 * 0, 9, 9);
					}
				}
				...
				for (int i = 0; i < len; i++) {
					if (i < 10) {
						instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(X + i * 10), Conversions.toScreenY(Y + addShake(shake)), 9 * 4, 9 * 0, 9, 9);
					} else {
						instance.ingameGUI.drawTexturedModalRect(Conversions.toScreenX(X + (i - 10) * 10), Conversions.toScreenY(Y - 10 + addShake(shake)), 9 * 4, 9 * 0, 9, 9);
					}
				}
				if (half == true) {
