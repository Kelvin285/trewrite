...
public class SnowballItemMixin extends Item {
    public SnowballItemMixin(Item.Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            ...
            if (observed_pos != null) {
                ...
                snowballEntity.setProperties(user, (float)pitch, (float)Math.toDegrees(Math.atan2(observed_pos.z - user.getZ(), observed_pos.x - user.getX())) - 90, 0.0F, 0.5F, 1.0F);
