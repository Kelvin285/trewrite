    ...
    public boolean isInRangeToRenderDist(double distance) {
	 ...
	 public void tick() {
		 ...
		 for (int i = 0; i < dragging.size(); i++) {
			 PlayerEntity player = dragging.get(i);
			 if (player.getPositionVec().distanceTo(getPositionVec()) >= 50) {
				 double px = Util.lerp(player.posX, posX, 0.1);
				 double py = Util.lerp(player.posY, posY, 0.1);
				 double pz = Util.lerp(player.posZ, posZ, 0.1);
				 player.attemptTeleport(px, py, pz, true);
			 } else {
				 this.dragging.remove(i);
			 }
		 }
