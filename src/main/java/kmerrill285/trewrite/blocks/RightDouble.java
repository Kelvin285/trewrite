...
public class RightDouble extends BlockT {

	   public static final DirectionProperty FACING = HorizontalBlock.HORIZONTAL_FACING;
	   public static final EnumProperty<DoubleBlockHalf> HALF = BlockStateProperties.DOUBLE_BLOCK_HALF;

	
	public RightDouble(float hardness, float difficulty, String name) {
...
//	            Block.spawnDrops(blockstate, worldIn, blockpos, (TileEntity)null, player, itemstack);
	   ...
	   public BlockState getStateForPlacement(BlockItemUseContext context) {
	      ...
	      if (blockpos.getY() < 255 && context.getWorld().getBlockState(blockpos.up()).isReplaceable(context)) {
	         ...
	         BlockPos pos2 = blockpos.add(context.getPlacementHorizontalFacing().rotateY().getXOffset(), 0, context.getPlacementHorizontalFacing().rotateY().getZOffset());
	   ...
	   public boolean isValidPosition(BlockState state, IWorldReader worldIn, BlockPos pos) {
	      ...
	      int x = facing.rotateY().getXOffset();
	      int z = facing.rotateY().getZOffset();
	      if (state.get(HALF) == DoubleBlockHalf.LOWER) {
